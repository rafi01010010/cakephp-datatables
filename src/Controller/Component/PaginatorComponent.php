        ...
        if(isset($settings['datatable'])){
            ...
            if(isset($dataTableQuery['start']) && $dataTableQuery['length']){
                ...
                if($dataTableQuery['start'] != 0){
                    $request->query['page'] = intdiv($dataTableQuery['start'],$settings['limit']) + 1;//($dataTableQuery['start']%$settings['limit']);
                }

            }
        }
        $result = parent::paginate($object, $settings); // TODO: Change the autogenerated stub
        if(isset($settings['datatable'])){
